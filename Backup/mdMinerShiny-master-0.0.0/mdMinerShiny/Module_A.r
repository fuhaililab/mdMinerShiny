...
DownloadRPPAData <- function(traverseResultFile, saveFolderName, cancerType, assayPlatform = "mda_rppa_core", tissueType = NULL, inputPatientIDs = NULL, outputFileName = "") 
{
  ...
  if (length(GeneNameColID) == 0)
  {
    GeneNameColID = grepBeginning(pattern = toupper("Gene.Name"), x = colnames(Annotation), ignore.case = TRUE);
  }
  REFColID = grepBeginning(pattern = toupper("Composite Element REF"), x = colnames(Annotation), ignore.case = TRUE);
  if (length(REFColID) == 0)
  {
    REFColID = grepBeginning(pattern = toupper("Protein.Name"), x = colnames(Annotation), ignore.case = TRUE);
  }
  ...
  {
    IDi = union(which(toupper(Annotation[, 2]) == toupper(left_columns)[i]),
                which(toupper(Annotation[, 2]) == sapply(strsplit(toupper(left_columns), split = "-"), 
                function(x){le = length(x);
                  if (le < 3) {str = "";} else {
                  str = paste(x[1:(le-2)], collapse = "-");}
                str;})[i]));
    if (length(IDi) == 0)
    {
